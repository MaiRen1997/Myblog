---
title: Go的map-(复合)数据类型
date: 2025-06-13 11:19:20
permalink: /pages/1bf8bd/
categories:
  - golang
  - Golang基础
tags:
...
3. map类型的变量默认初始值为nil，需要使用make()函数来分配内存。语法为：

4. make：用于slice、map和channel的初始化
...
1. 我们在获取map的时候，会返回两个值，也可以是返回的结果，一个是是否有该元素
...
1. 使用delete()内建函数从map中删除一组键值对，delete函数的格式如下所示
...
1. 我们想要在切片里面存放一系列用户的信息，这时候我们就可以定义一个元素为map类型的切片
