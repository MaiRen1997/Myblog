...
`Field()` 是 Pydantic 库中的一个函数，用于**增强模型字段的配置能力**。它让你可以为字段添加验证规则、默认值、文档说明等。
...
class Student(BaseModel):
    # 大于等于 0
    score: float = Field(ge=0, description="分数不能为负数")
    
    # 大于 0
    height: float = Field(gt=0, description="身高必须大于0")
    
    # 小于等于 100
    percentage: Optional[float] = Field(le=100)
...
# 测试
try:
    s = Student(score=-5, height=170, percentage=101, age=200, temperature=50)
...
一般通过`Optional[类型]` 来使用，表示只能是中括号里的类型，或者是None

```python
age: Optional[float] = Field(le=100)
```
