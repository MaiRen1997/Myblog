...
1. Python中，我们可以动态为对象添加属性，这是Python作为动态类型语言的一项特权
...
1. 如果不希望在使用对象时动态的为对象添加属性，可以使用Python的`__slots__`魔法。

2. 对于`Student`类来说，可以在类中指定`__slots__ = ('name', 'age')`，这样`Student`类的对象只能有`name`和`age`属性，

3. 如果想动态添加其他属性将会引发异常，代码如下所示:
...
2. 参数说明
   ...
   - **object**：要获取属性的对象
   - **name**：字符串形式的属性名
   - **default**（可选）：如果属性不存在，返回的默认值（不提供则抛出 AttributeError）

3. 代码示例

   ```python
   obj = Human('barry','男',18,'运动')
   print(getattr(obj, 'mind', '空'))
   ```

4. 优点

   1. 使用`点`的方式，访问不存在的属性，会存在报错
   2. **使用getattr访问不存在的属性，使用的是默认值**
