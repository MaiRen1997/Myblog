---
title: 类的成员
date: 2025-05-07 14:02:22
permalink: /pages/fa1849/
categories:
  - 后端
  - Python学习
  - 面向对象编程
...
- 私有静态字段：仅类内部可以访问；`以双下划线开头`，类外访问需特殊语法
- 保护静态字段： `以单下划线开头`，约定为“不要直接访问”
...
2. 静态属性，**类内部可以访问，外部不可访问**

3. 如果外部要访问，可以通过访问类的方式访问
   ...
   2. 注意，通过下划线访问，是直接拼接类名，而不是通过`点的方式`
...
1. Python中可以通过`property`装饰器为“私有”属性提供读取和修改的方法，之前我们提到过，装饰器通常会放在类、函数或方法的声明之前，通过一个`@`符号表示将装饰器应用于类、函数或方法

2. 使用@property定义get方法

3. 使用@变量名.setter定义set方法
...
如`__init__`,是python内置的特殊属性，不要自定义这类名称
